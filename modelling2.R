...
########################################################################################################################################
  ...
  varimp_res_goof <- function(){
    ...
    r.models <- list.files(paste0(save_dir, '/', depths[d]), pattern = "xvMod_", full.names=T)
    fit_model <- readRDS(r.models[1])
    preddf <- predict(fit_model, newdata=cDat)
    for (i in 2:nbags){
      fit_model <- readRDS(r.models[i])
      pred <- predict(fit_model, newdata=cDat)
    ...
    r.models <- list.files(paste0(save_dir, '/', depths[d]), pattern = "xvMod_", full.names=T)
    fit_model <- readRDS(r.models[1])
    preddf <- predict(fit_model, newdata=vDat)
    mseVec <- mse(vDat[,1], preddf)
    for (i in 2:nbags){
      fit_model <- readRDS(r.models[i])
      pred <- predict(fit_model, newdata=vDat)
      preddf <- cbind(preddf, pred)
  ...
  if(method_dsm == 'qrf'){
    ...
    lMat <- predict(fit_model$finalModel, vDat, type='quantiles', quantiles=q4pilb)$predictions
    write.csv(lMat, paste0(save_dir, '/', depths[d], '/lMat.csv'), row.names=F)
    uMat <- predict(fit_model$finalModel, vDat, type='quantiles', quantiles=q4piub)$predictions
    write.csv(uMat, paste0(save_dir, '/', depths[d], '/uMat.csv'), row.names=F)

    # assess goodness of fit - calibration and validation data
    cDat_GOOF <- data.frame('Depth' = depths[d], 'Obs' = cDat[,1], 'Pred' = predict(fit_model, newdata=cDat))
    write.csv(cDat_GOOF, paste0(save_dir, '/', depths[d], '/cDat_GOOF.csv'), row.names=F)

    vDat_GOOF <- data.frame('Depth' = depths[d], 'Obs' = vDat[,1], 'Pred' = predict(fit_model, newdata = vDat))
    write.csv(vDat_GOOF, paste0(save_dir, '/', depths[d], '/vDat_GOOF.csv'), row.names=F)
