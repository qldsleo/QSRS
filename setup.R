...
map_now       <- FALSE   # step 3: will run mapping script
mosaic_now    <- TRUE   # step 4: will run mosaic script
...
covs_subset_cat <- c('Clim_GEOSS_Aus_Macroclimate_Bioclimatic_zones_qld.tif', 'PM_GEOSS_Aus_Lithology_Lithology_qld.tif',
                     ...
                     'Soil_QSRS_qld.tif', 'Landuse_qld.tif')
...
if (version == 1){
  ...
  covs_dropped <- ''
  if (any(attribute == c('tn','tp'))){
    covs_subset <- c(covs_subset, 'Landuse_qld.tif')
  }
}
...
depths <- c(names(data[-(1:3)]), 'all') # list of depths in training data
...
rmarkdown2 <- paste0(code, 'QSRS/QSRS_all.Rmd')
