...
```{r cDat table, echo=F}
cDat <- read.csv(paste0(resd, '/models/', ifelse(run_PCA,'PCA/','all/'), 'GOOFData.csv'))
paged_table(cDat[1:6,1:10])
```

```{r cDat, echo=F}
plot_data <- do.call(rbind, lapply(1:6, function(i){
  read.csv(paste0(resd, 'models/', ifelse(run_PCA,'PCA/','all/'), depths[i], '/cDat_GOOF.csv'))
}))
...
```{r vDat table, echo=F}
vDat <- read.csv(paste0(resd, '/models/', ifelse(run_PCA,'PCA/','all/'), 'GOOFData.csv'))
paged_table(vDat[1:6, c(1,11:19)])
```

```{r vDat, echo=F}
plot_data <- do.call(rbind, lapply(1:6, function(i){
  read.csv(paste0(resd , 'models/', ifelse(run_PCA,'PCA/','all/'), depths[i], '/vDat_GOOF.csv'))
}))
plot_data$Depth = factor(plot_data$Depth, levels=c('X0.5.cm','X5.15.cm','X15.30.cm','X30.60.cm','X60.100.cm','X100.200.cm'))
ggplot(plot_data, aes(Obs, Pred))+
  geom_point(size=0.5)+geom_abline(col='red', size=0.75)+geom_smooth(method='lm',formula='y ~ x', col='blue', size=0.75)+
...
## Model Fitting Results - Depth as Covariate

The following results are from the generation of a single model using depth as a covariate.
...
```{r cDat all table, echo=F}
cDat <- read.csv(paste0(resd, '/models/', ifelse(run_PCA,'PCA/','all/'), 'GOOFData.csv'))
paged_table(cDat[7:12,1:10])
```

```{r cDat all, echo=F}
plot_data <- read.csv(paste0(resd , 'models/', ifelse(run_PCA,'PCA/','all/'), depths[7], '/cDat_GOOF.csv'))
...
```{r vDat all table, echo=F}
vDat <- read.csv(paste0(resd, '/models/', ifelse(run_PCA,'PCA/','all/'), 'GOOFData.csv'))
paged_table(vDat[7:12, c(1,11:19)])
```

```{r vDat all, echo=F}
plot_data <- read.csv(paste0(resd , '/models/', ifelse(run_PCA,'PCA/','all/'), depths[7], '/vDat_GOOF.csv'))
plot_data$Depth = factor(plot_data$Depth, levels=c('2.5','10','22.5','45','80','150'))
ggplot(plot_data, aes(Obs, Pred))+
  geom_point(size=0.5)+geom_abline(col='red', size=0.75)+geom_smooth(method='lm',formula='y ~ x', col='blue', size=0.75)+
  facet_wrap(~ Depth, scales='free', labeller=labeller(Depth=c('2.5'='X0.5.cm','10'='X5.15.cm','22.5'='X15.30.cm',
...
```{r vDat all varimp, echo=F}
if(names(ovi[1]) != 'Covariate'){ovi <- rename(ovi, Covariate = Row.names)}
ovi$Covariate <- gsub('_qld', '', ovi$Covariate)
ovi <- ovi[order(-ovi$all),] # Sort in descending order of mean importance
paged_table(ovi[order(-ovi$all),][c(1,8)])
```